...
| [0003. Remote SSH extensions](0003-remote-ssh-extensions.md) | Accepted | 2026-04-08 | Remote port detection + forwarding, a TCP proxy, OSC 7457 remote-browser open, and `$BROWSER` injection, carried over new mux PDUs and fork-only modules. |
...
| [0006. open-url security policy](0006-open-url-security-policy.md) | Accepted | 2026-04-09 | Remote-initiated URL opens default to `Confirm`, gated by an allow-list, with per-domain overrides — no silent browser launches from remote hosts. |
| [0007. Windows rendering modes and window-state persistence](0007-windows-rendering-modes-and-state-persistence.md) | Accepted | 2026-05-24 | Three auto-selected render modes (WgpuDComp / WgpuClassic / SoftwareRdp) for RDP/GPU correctness, plus window-state persistence, validated by the `tests/ux/` harness. |
